...
VALUES ('alpha_cyl_t', 'mi3272.alpha_cyl_t', 'коэффициент линейного расширения материала цилиндра компакт-прувера, ºC-1. Значение берут из таблицы Б.1 приложения Б. Если этот коэффициент указан в паспорте или техническом описании на компакт-прувер (или в заводском сертификате калибровки компакт-прувера, то значение нужно брать оттуда. Либо вместо этого коэффициента можно прислать alpha_cyl_t_sq', TRUE, 'mi3272'),
    ...
    ('T_ij_coef', 'mi3272.T_ij_coef', 'время прохода поршнем калиброванного участка компакт-прувера по 3.1.5 при i-том измерении при установлении расхода в j-той точке расхода, с. Используется для вычисления расхода для определения коэффициента преобразования ТПР (таблица 2 часть 1).', TRUE, 'mi3272'),
    ('rho_BIK_ij_coef', 'mi3272.rho_BIK_ij_coef', 'плотность рабочей жидкости, измеренная ПП, установленным в БИК, при i-м измерении в j-ой точке расходе, кг/м3. Используется для вычисления расхода (таблица 2 часть 1).', TRUE, 'mi3272'),
    ('t_PP_ij_coef', 'mi3272.t_PP_ij_coef', 'температура рабочей жидкости в поточном ПП, ºC. Используется для вычисления расхода (таблица 2 часть 1).', TRUE, 'mi3272'),
    ('P_PP_ij_coef', 'mi3272.P_PP_ij_coef', 'давление рабочей жидкости в поточном ПП, Мпа. Используется для вычисления расхода для определения коэффициента преобразования ТПР (таблица 2 часть 1).', TRUE, 'mi3272'),
    ('rho_PP_ij_coef', 'mi3272.rho_PP_ij_coef', 'плотность рабочей жидкости, измеренной поточным ПП. Используется для вычисления расхода для определения коэффициента преобразования ТПР (таблица 2 часть 1).', TRUE, 'mi3272'),
    ...
    ('W_w_TPR_ij', 'mi3272.W_w_ij', 'объемная доля воды в нефти (сырой), %. Используется для расчета таблицы 2 часть I', TRUE, 'mi3272'),
    ('W_xc_TPR_ij', 'mi3272.W_xc_ij', 'объемная доля хлористых солей в пластовой воде (в воде, содержащейся в сырой нефти), определенная по результатам анализа объединенной пробы сырой нефти в химико-аналитической лаборатории, %. Используется для расчета таблицы 2 часть I', TRUE, 'mi3272'),
