...
VALUES ('measureCount', 'mi3272.measureCount', 'кол-во измерений (measureCount >=5)', TRUE, 'mi3272'),
    ...
    С помощью этого параметра вычисляются beta_fluid_ij и gamma_fluid_ij', TRUE, 'mi3272'),
       ('T_ij_TPR', 'mi3272.T_ij_TPR', 'время прохода поршнем калиброванного участка компакт-прувера по 3.1.5 при i-том измерении при установлении расхода в j-той точке расхода, с. Используется для вычисления расхода для определения коэффициента преобразования ТПР (таблица 2 часть 1).', TRUE, 'mi3272'),
       ...
       ('alpha_st_t', 'mi3272.alpha_st_t',
        ...
        TRUE, 'mi3272'),
       ('rho_TPR_ij', 'mi3272.rho_TPR_ij', 'плотность, измеренная при определения коэффициента преобразования ТПР. Используется только для определения beta и gamma в формуле (7)',
        TRUE, 'mi3272'),
       ('rho2_TPR_ij', 'mi3272.rho2_TPR_ij', 'плотность, измеренная при определения коэффициента преобразования ТПР. Используется только для определения beta и gamma в формуле (7). Для повтороного подсчета коэффициента преобразования ТПР',
        TRUE, 'mi3272'),
       ('rho_PP_ij', 'mi3272.rho_PP_ij', 'плотность рабочей жидкости, измеренной поточным ПП, участвующим в поверке',
        TRUE, 'mi3272'),
